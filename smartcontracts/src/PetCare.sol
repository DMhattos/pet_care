...
contract PetCare {    
    ...
    uint private petIdCounter = 1;                 // Inicializa o contador de ID para pets
    mapping(uint => Pet) public pets;              // Mapeia ID do pet para a estrutura Pet
    mapping(uint => Vaccine[]) public petVaccines; // Mapeia ID do pet para uma lista de vacinas

    event PetAdded(uint petId, string name, address owner);
    event VaccineAdded(uint petId, string vaccineName);
    ...
    function addVaccineToPet(uint _petId, string memory _vaccineName, string memory _dateAdministered) public {
    ...
    function getVaccineCountForPet(uint _petId) public view returns (uint) {
        return petVaccines[_petId].length;
    }

    // Função para obter os detalhes de uma vacina específica de um pet
    function getVaccineDetails(uint _petId, uint _vaccineIndex) public view returns (string memory name, string memory dateAdministered) {
    ...
    function getPetDetails(uint _petId) public view returns (string memory name, string memory breed, uint8 age, address owner) {
